...
## FR — Utilisation dans un Jenkinsfile

### Option 1 — Librairie globale (configurée dans Jenkins UI)
```groovy
    ...
    stages {
        stage("build and push") {
            steps {
                script {
                    buildJar()
                    buildImage('mb938/demo-app:1.0')
                    dockerLogin()
                    dockerPush('mb938/demo-app:1.0')
                }
            }
        }
    }
...
### Option 2 — Librairie scoped au projet (sans configuration Jenkins admin)
```groovy
library identifier: 'jenkins-shared-library@main', retriever: modernSCM(
    [$class: 'GitSCMSource',
    remote: 'https://github.com/m-bengueddache/jenkins-shared-library.git',
    credentialsId: 'git-credentials']
)
```

## EN — Usage in a Jenkinsfile

### Option 1 — Global library (configured in Jenkins UI)
```groovy
@Library('jenkins-shared-library') _
...
### Option 2 — Project-scoped library (no Jenkins admin config needed)
```groovy
library identifier: 'jenkins-shared-library@main', retriever: modernSCM(
    [$class: 'GitSCMSource',
    remote: 'https://github.com/m-bengueddache/jenkins-shared-library.git',
